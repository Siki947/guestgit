...
def add_event(request):
    #通过POST请求接收发布会参数：发布会id、标题、人数、状态、地址和时间等
    ...
    #判断eid,name,limit,address,start_time等字段均不能为空，否则JsonResponse()返回相应的状态码和提示
    ...
    if status == '':
        status = 1
    #将数据插入到Event表，插入过程中如果日期格式错误，将抛出ValidationError异常
    try:
...
def add_guest(request):
    ...
    logging.info ('event_time=%d' %event_time)
    etime = str(event_time).split('.')[0]
    logging.info('etime=%d' %etime)
    timeArray = time.strptime(etime,"%Y-%m-%d %H:%M:%S")
    logging.info('timeArray=%d' %timeArray)
    e_time = int(time.mktime(timeArray))
    logging.info('e_time=%d' %e_time)

    now_time = str(time.time())  #当前时间
    logging.info('logging.info=%d' %now_time)
    ntime = now_time.split(".")[0]
    logging.info('ntime=%d' %ntime)
    n_time = int(ntime)
